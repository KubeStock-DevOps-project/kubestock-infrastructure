...
echo "Run: chmod +x 2-deploy-cluster.sh && ./2-deploy-cluster.sh"
echo ""

# SSH into dev server and cd to script directory
exec ssh -o StrictHostKeyChecking=no -i "$SSH_KEY" -t ${SSH_USER}@${DEV_SERVER_IP} "cd ~/kubestock-core/infrastructure/terraform/demo && exec bash"
